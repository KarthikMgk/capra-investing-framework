...
- No App.tsx — TanStack Router file-based routing; route at `src/routes/_layout/upload.tsx` with same admin beforeLoad guard as admin.settings
- routeTree.gen.ts auto-regenerated by Vite plugin when upload.tsx was created
- `createUploadMutation` factory reduces duplication between screener and RBI hooks
- FileDropZone uses both onDrop (drag) and input onChange (click) — both validate .csv extension
- ValidationResult reads `error.error.details.expected/found` for column-level display; falls back to generic message for other error codes
- UploadSection internal component keeps two sections fully independent (separate useState, separate mutation)
- 4/4 test cases cover all AC9 scenarios
...
- `frontend/src/features/upload/UploadScreen.tsx` (new)
- `frontend/src/features/upload/UploadScreen.test.tsx` (new)
- `frontend/src/routes/_layout/upload.tsx` (new)
